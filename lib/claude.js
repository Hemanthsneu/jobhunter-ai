...
const ClaudeAI = {
    ...
    async analyzeJobMatch(apiKey, resume, jobDescription, userProfile = {}) {
        const systemPrompt = `You are a precise job match analyzer. Respond ONLY with valid JSON. No markdown, no explanation, no code blocks.
JSON schema: {
...
Respond with ONLY the JSON object.`;
    ...
    async tailorResume(apiKey, resume, jobDescription, template = 'jakes', userProfile = {}) {
        const systemPrompt = `You are an elite resume writer and career strategist who has hired 500+ engineers at top companies. You rewrite resumes to be ATS-optimized AND compelling to human reviewers.
RULES:
1. NEVER fabricate experience, titles, companies, or metrics
2. Mirror exact keywords from the job description
3. Every bullet: [Strong Verb] + [Specific Action] + [Quantified Result]
4. Prioritize the 3 most relevant experiences at the top
5. Maximum 1 page, 480-520 words
6. Order skills section by frequency in job description
7. Preserve all real numbers/metrics, improve vague ones with context
8. Remove anything not relevant to this specific role
9. CONTACT INFO: Only include contact details from the original resume. If LinkedIn, GitHub, phone, or email are NOT in the original resume, do NOT add placeholders. Use pipe "|" separator between contact items.
Respond with ONLY the resume text. No commentary, no markdown.`;
...
Generate the COMPLETE tailored resume in plain text, following the template format. Output ONLY the resume content.`;

        return await this.call(apiKey, prompt, systemPrompt, { model: 'sonnet', maxTokens: 8192 });
    },

    async calculateATSScore(apiKey, resume, jobDescription) {
        const systemPrompt = `You are an ATS (Applicant Tracking System) simulator. Analyze resumes exactly how automated systems do. Respond ONLY with valid JSON. No markdown, no code blocks.`;
...
Respond with ONLY this JSON:
{
  ...
  "improvements": [
    {"action": "specific action", "impact": "expected improvement", "priority": 1}
  ],
