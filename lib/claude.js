...
Remember: include ALL jobs, ALL education, ALL projects, ALL skills from the original. Output ONLY the resume.`;
    ...
    // LaTeX-specific tailoring — outputs only body sections in LaTeX format
    async tailorResumeLatex(apiKey, resume, jobDescription, userProfile = {}) {
        const systemPrompt = LaTeXTemplates.getLatexTailoringPrompt();

        const prompt = `Tailor this resume for the target job. Output ONLY LaTeX body sections (Summary, Technical Skills, Experience, Projects and Hackathons).

DO NOT output: \\documentclass, \\usepackage, \\begin{document}, \\end{document}, heading/contact info, Education section, or Certifications section. Those are handled separately.
...
Candidate: ${userProfile.name || 'Hemanth Saragadam'} | Target: ${userProfile.role || 'Software Engineer'}

Output the following sections ONLY, using the exact LaTeX commands:
1. \\section{Summary} with \\resumeItem
2. \\section{Technical Skills} with \\textbf{Category:} values
3. \\section{Experience} with \\resumeSubheading and \\resumeItem bullets
4. \\section{Projects and Hackathons} with \\resumeProjectHeading and \\resumeItem bullets

Include ALL jobs and ALL projects from the original. Reorder bullets by relevance to the job. Output ONLY valid LaTeX code, no commentary.`;
