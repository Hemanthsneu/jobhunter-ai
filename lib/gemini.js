...
const GeminiAI = {
    ...
    async call(apiKey, prompt, systemInstruction = '') {
...
Respond in this exact JSON format:
{
  ...
  "summary": "2-3 sentence overall assessment"
    ...
    async tailorResume(apiKey, resume, jobDescription, template = 'jakes', userProfile = {}) {
        const systemInstruction = `You are an expert resume writer who has helped thousands of engineers land FAANG jobs. You specialize in ATS optimization and know exactly how to format resumes to get 95%+ ATS scores. You always preserve the candidate's authentic experience while optimizing language and keywords.`;

        const templateInstructions = this.getTemplateInstructions(template);

        const prompt = `Tailor this resume for the specific job description below. Your goal is to achieve an ATS score of 95+.

RULES:
1. Keep all information truthful - only rephrase, don't fabricate
2. Prioritize keywords from the job description naturally
3. Use strong action verbs and quantified achievements
4. Match the job's required skills language exactly (e.g., if JD says "React.js", don't write "React")
5. Front-load most relevant experience
6. Remove irrelevant details to make room for relevant ones
7. Ensure every bullet point has: Action Verb + Task + Result/Impact + Metric
...
- Years of Experience: ${userProfile.yoe || '5'}
...
Generate the tailored resume in plain text following the template format exactly. Make it ATS-optimized with 95%+ score potential. Include all sections.`;

        return await this.call(apiKey, prompt, systemInstruction);
    },
